-- Manual fix for stuck offers - Run this to fix offers that are stuck at Stage 1
...
SET 
    ...
    END,
    status = CASE 
        WHEN s.investment_advisor_code IS NOT NULL THEN 'pending_startup_advisor_approval'
        ELSE 'pending_startup_review'
    END,
    updated_at = NOW()
FROM startups s
