...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // First check if startup already exists on TMS - check user_profiles instead of users
    ...
    if (existingStartupByEmail) {
      ...
      userId = existingStartupByEmail.auth_user_id; // Use auth_user_id, not profile id
      isExistingTMSStartup = true;
      console.log('Startup already exists on TMS, permission required:', userId);

      // Find startup record (user_id in startups table is auth_user_id)
      ...
      if (startupRecord) {
        ...
        if (existingStartupByEmail.investment_advisor_code_entered === advisorCode) {
          ...
          // advisorId is auth_user_id, so find their Investment Advisor profile
