...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void | VercelResponse> {
  ...
  try {
    ...
    // AUTO-RENEW ADVISOR CREDITS (triggered by GitHub Actions cron)
    if (req.body.endpoint === 'auto-renew-advisor-credits') {
      ...
      try {
        ...
        for (const assignment of expiredAssignments) {
          try {
            // Check advisor's available credits
            ...
            if (advisorCredits.credits_available >= 1) {
              ...
              // Extend startup's subscription by 1 month
              ...
              if (updateSubError) {
                ...
                // Get current subscription details for audit
                ...
                if (subDetails) {
                  ...
                  // Insert billing_cycle record for audit
                  await supabase
                    .from('billing_cycles')
                    .insert({
                  ...
                  // Update billing_cycle_count
                  await supabase
                    .from('user_subscriptions')
                    .update({ billing_cycle_count: nextCycleNumber })
                    .eq('id', assignment.subscription_id);
                }
              }
              ...
              // NO CREDITS: Disable auto-renewal
