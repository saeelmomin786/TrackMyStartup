...
import { generateInvestorCode, generateInvestmentAdvisorCode, generateMentorCode, generateFacilitatorCode } from './utils'
...
export const authService = {
  ...
  async signUp(data: SignUpData & { founders?: Founder[]; fileUrls?: { [key: string]: string } }): Promise<{ user: AuthUser | null; error: string | null; confirmationRequired: boolean }> {
    try {
      ...
      if (authData.user && authData.user.email_confirmed_at) {
        ...
        const facilitatorCode = data.role === 'Startup Facilitation Center' ? generateFacilitatorCode() : null;
  ...
  async createProfile(profileData: {
    ...
    try {
      ...
      const facilitatorCode = profileData.role === 'Startup Facilitation Center' ? generateFacilitatorCode() : null;
