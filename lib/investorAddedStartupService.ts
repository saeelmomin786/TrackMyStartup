...
export interface InvestorAddedStartup {
  id: number;
  investor_id: string;
  startup_name: string;
  sector?: string;
...
export interface CreateInvestorAddedStartup {
  investor_id: string;
  startup_name: string;
  sector?: string;
  website_url?: string;
...
export interface UpdateInvestorAddedStartup {
  startup_name?: string;
  sector?: string;
  website_url?: string;
  linkedin_url?: string;
...
class InvestorAddedStartupService {
  // Get all startups added by an investor
  async getStartupsByInvestor(investorId: string): Promise<InvestorAddedStartup[]> {
    try {
      ...
      if (error) {
        console.error('Error fetching investor added startups:', error);
        throw error;
      }

      return data || [];
    } catch (error) {
      console.error('Error in getStartupsByInvestor:', error);
      return [];
    }
  }

  // Get a single startup by ID
  async getStartupById(startupId: number): Promise<InvestorAddedStartup | null> {
    try {
      const { data, error } = await supabase
        .from('investor_added_startups')
        .select('*')
  ...
  async createStartup(startup: CreateInvestorAddedStartup): Promise<InvestorAddedStartup | null> {
    try {
  ...
  async updateStartup(startupId: number, updates: UpdateInvestorAddedStartup): Promise<InvestorAddedStartup | null> {
    try {
      const { data, error } = await supabase
        .from('investor_added_startups')
        .update(updates)
...
export const investorAddedStartupService = new InvestorAddedStartupService();
