...
import autoRenewAdvisorCreditsHandler from './autoRenewAdvisorCredits';
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void | VercelResponse> {
  ...
  try {
    ...
    // AUTO-RENEW ADVISOR CREDITS (GitHub Actions POST → same handler as Vercel Cron GET)
    if (req.body.endpoint === 'auto-renew-advisor-credits') {
      return autoRenewAdvisorCreditsHandler(req, res);
    }
