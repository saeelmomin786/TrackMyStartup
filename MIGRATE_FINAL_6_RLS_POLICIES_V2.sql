-- =====================================================
-- MIGRATE FINAL 6 RLS POLICIES (V2 - Based on Actual Expressions)
-- OPTIMIZED VERSION: NO FALLBACK (FASTER FOR LARGE USER BASE)
-- =====================================================
-- This fixes the exact patterns found in the actual policy expressions
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            -- =====================================================
            -- Replace patterns based on actual expressions
            -- =====================================================
            IF new_qual IS NOT NULL THEN
                -- Replace JOIN users u ON ((u.id = s.user_id))
                new_qual := regexp_replace(new_qual, 
                    'JOIN\s+users\s+u\s+ON\s+\(\(u\.id\s*=\s*s\.user_id\)\)', 
                    'JOIN public.user_profiles u ON ((u.auth_user_id = s.user_id))',
                    'gi');
                
                -- Replace FROM users u (with alias)
                new_qual := regexp_replace(new_qual, 
                    'FROM\s+users\s+u\b', 
                ...
                -- Replace any remaining FROM users (without alias)
                new_qual := regexp_replace(new_qual, 
                    '\bFROM\s+users\b(?!\s+u\b)', 
